...
export async function getNextSundayTatort() {
  ...
  const shows = $(".broadcastBox").map(function (this: Cheerio) {
    ...
    return {
      time: showHours
        ? new Date(
            new Date(nextSunday.getTime()).setHours(
              showHours[0],
              showHours[1],
              0,
              0
...
export async function getTodayTatort() {
  const today = new Date();
  const month = `0${today.getMonth() + 1}`.slice(-2);
  const day = `0${today.getDate()}`.slice(-2);

  const url = `https://www.daserste.de/programm/index~_pd-${today.getFullYear()}${month}${day}.html`;
  const $ = cheerio.load(await fetch(url).then((response) => response.text()));
  ...
  const shows = $(".broadcastBox").map(function (this: Cheerio) {
    ...
    return {
      time: showHours
        ? new Date(
            new Date(today.getTime()).setHours(showHours[0], showHours[1], 0, 0)
          )
        : undefined,
      name: clean($(this).find(".headline").text()),
      title: clean($(this).find(".dachzeile").text()),
